...
PROGNAME=`basename $0`
VERSION="Version 1.0p1,"
AUTHOR="2009, Mike Adolphs (http://www.matejunkie.com/), addtion of 'steal' added by David Phillips, 2013"
...
get_cpuvals() {
	...
	tmp1_cpu_steal=`grep -m1 '^cpu' /proc/stat|awk '{print $9}'`
	tmp1_cpu_total=`expr $tmp1_cpu_user + $tmp1_cpu_nice + $tmp1_cpu_sys + \
$tmp1_cpu_idle + $tmp1_cpu_iowait + $tmp1_cpu_irq + $tmp1_cpu_softirq + $tmp1_cpu_steal`
	...
	tmp2_cpu_steal=`grep -m1 '^cpu' /proc/stat|awk '{print $9}'`
	tmp2_cpu_total=`expr $tmp2_cpu_user + $tmp2_cpu_nice + $tmp2_cpu_sys + \
$tmp2_cpu_idle + $tmp2_cpu_iowait + $tmp2_cpu_irq + $tmp2_cpu_softirq + $tmp2_cpu_steal`
	...
	diff_cpu_steal=`echo "${tmp2_cpu_steal} - ${tmp1_cpu_steal}" | bc -l`
...
| bc -l | sed 's/^\./0./'`
	...
	cpu_steal=`echo "scale=2; (1000*${diff_cpu_steal}/${diff_cpu_total}+5)/10" | bc -l | sed 's/^\./0./'`
	cpu_total=`echo "scale=2; (1000*${diff_cpu_total}/${diff_cpu_total}+5)\\
/10" | bc -l | sed 's/^\./0./'`
	cpu_usage=`echo "(${cpu_user}+${cpu_nice}+${cpu_sys}+${cpu_iowait}+\\
${cpu_irq}+${cpu_softirq}+${cpu_steal})/1" | bc`
}

do_output() {
	output="user: ${cpu_user}, nice: ${cpu_nice}, sys: ${cpu_sys}, \
iowait: ${cpu_iowait}, irq: ${cpu_irq}, softirq: ${cpu_softirq}, steal: ${cpu_steal} \
idle: ${cpu_idle}"
}

do_perfdata() {
	perfdata="'user'=${cpu_user} 'nice'=${cpu_nice} 'sys'=${cpu_sys} \
'softirq'=${cpu_softirq} 'iowait'=${cpu_iowait} 'irq'=${cpu_irq} 'steal'=${cpu_steal} \
'idle'=${cpu_idle}"
}
